...
from .. import utils
from ..utils import concat_tensors,  apply_method, checklist
...
class RNNLayer(nn.Module):
    ...
    nn_lin = "relu"
    dump_patches = True
    def __init__(self, input_dim, output_dim, nn_lin="relu", normalization='batch', dropout=None, name_suffix="", *args,
                 **kwargs):
        ...
        #     modules["normalization_" + name_suffix] = nn.BatchNorm1d(output_dim)
        # if self.normalization == 'instance':
        #     modules["normalization_" + name_suffix] = nn.InstanceNorm1d(1)
    ...
    def forward(self, x, previous_h=None, retain_hidden=False):
        if len(x.shape) > 3:
            x = flatten(x, 2)
        previous_h = previous_h if previous_h is not None else self.previous_h
        out, h = self.module[0](x, previous_h)
        # if self.normalization is not None:
            # out = self.module._modules['normalization_' + self.name_suffix](out)
...
class LSTMLayer(RNNLayer):
    ...
    def forward(self, x, previous_h = None, previous_c = None, retain_hidden=False):
        ...
        if self.normalization:
            if self.normalization == 'batch':
                out_h = self.module._modules['normalization_' + self.name_suffix](out_h)
            elif self.normalization == 'instance':
                out_h = self.module._modules['normalization_' + self.name_suffix](out_h.unsqueeze(1))
                out_h = out_h.squeeze()
        '''
        if self.nn_lin:
            out = self.module._modules['nnlin' + self.name_suffix](out)
...
class RVAEEncoder(HiddenModule):
    ...
    def __init__(self, pins, phidden={}, pouts=None, recurrent_params={}, make_flows=None, *args, **kwargs):
        ...
        if not has_hidden:
            self._hidden_modules = Sequential(self.make_recurrent_layer(pins, recurrent_params, *args, **kwargs))
...
class RVAEDecoder(HiddenModule):
    ...
    def __init__(self, pins, phidden={}, pouts=None, recurrent_params={}, *args, **kwargs):
        ...
        if not has_hidden:
            self._hidden_modules = nn.Sequential(self.make_recurrent_layer(pins, recurrent_params, *args, **kwargs))
        if pouts:
