...
import numpy as np
from ..utils import checklist, checktuple, flatten_seq_method, oneHot, apply
from . import init_module, MLP_DEFAULT_NNLIN, CONDITIONING_HASH, flatten
...
class MLPLayer(nn.Module):
    ...
    def __init__(self, input_dim, output_dim, nn_lin=None, batch_norm='batch', dropout=None, name_suffix="", bias=True, *args, **kwargs):
        ...
        self.name_suffix = name_suffix; self.batch_norm = batch_norm 
...
class MLPGatedLayer(MLPLayer):
    ...
    if the input tensor has more than two dimensions, the tensor is flattened over the first dim.
    """
    def __init__(self, input_dim, output_dim, batch_norm='batch', *args, **kwargs):
        super(MLPGatedLayer, self).__init__(input_dim, output_dim, batch_norm=None, *args, **kwargs)
...
class MLP(nn.Module):
    ...
    def get_input_dimension(self, pins, phidden):
        ...
        if is_conditioned and conditioning == 'concat':
            label_dim = sum([v['dim'] for v in is_conditioned.values()])
            input_dim += label_dim
        return input_dim

    def get_hidden_layers(self, pins, phidden={"dim":800, "nlayers":2, 'label':None, 'nn_lin':MLP_DEFAULT_NNLIN, 'conditioning':'concat', "linked":False}, name=""):
        ...
        LayerModule = checklist(phidden.get('layer', self.DefaultLayer), n=nlayers)
        ...
        phidden['batch_norm'] = checklist(phidden.get('batch_norm'), n=nlayers)
        ...
        for i in range(nlayers):
            ...
            modules['layer_%d'%i] = LayerModule[i](n_in, hidden_dim, nn_lin=phidden['nn_lin'][i],
                                                   batch_norm=phidden['batch_norm'][i], dropout=phidden['dropout'][i],
            ...
            if type(self.pins) == list:
                ...
                for i, ph in enumerate(self.phidden):
                    ...
                    if ph.get('label_params') and ph.get('conditioning', 'concat'):
                        ...
                        y_tmp = self.format_label_data(y, ph)
