...
from . import MLP
from . import init_module, get_init, CONV_DEFAULT_NNLIN
...
class ConvLayer(nn.Module):
    ...
    :param batch_norm: has batch normalization (None, 'batch' or 'instance', default : 'batch')
    ...
    def build_module(self, in_channels, out_channels, conv_dim, kernel_size, pool=None, dilation=1, dropout=0.5, padding=0, stride=1, windowed=None, nn_lin=CONV_DEFAULT_NNLIN, batch_norm=True, *args, **kwargs):
        ...
        if self.batch_norm:
            if self.batch_norm == 'batch':
                self.add_module('batchnorm_module', self.bn_modules[conv_dim](out_channels))
            elif self.batch_norm == 'instance':
                self.add_module('instancenorm_module', self.in_modules[conv_dim](out_channels))
        # Non-linearity
        self.nn_lin = nn_lin
        if self.nn_lin:
            self.add_module('nnlin_module', getattr(nn, nn_lin)())
        # Pooling layer
        self.pooling = pool
        if not pool is None:
    ...
    def forward(self, x):
        ...
        if self.batch_norm:
            if self.batch_norm == 'batch':
                current_out = self._modules['batchnorm_module'](current_out)
            elif self.batch_norm == 'instance':
                current_out = self._modules['instancenorm_module'](current_out)
...
class GatedConvLayer(ConvLayer):
    ...
    def build_module(self, in_channels, out_channels, conv_dim, kernel_size,  dilation=1, padding=0, stride=1, *args, **kwargs):
        super(GatedConvLayer, self).__init__(in_channels, out_channels, conv_dim, kernel_size, dilation=dilation, padding=padding, nn_lin=None, stride=stride, *args, **kwargs)
        padding = int(padding)
        self.pool_indices = None
    ...
    def init_modules(self, *args, **kwargs):
        ...
        torch.nn.init.constant_(self._modules['conv_module_residual'].weight, 1./(self.kernel_size*self.in_channels))
...
class Convolutional(nn.Module):
    ...
    * *batch_norm_conv* batch normalization (can be : None, 'batch', 'instance', default : 'batch')
    * *dropout_conv* dropout (default : 0)
    ...
    def __init__(self, pins, phidden, conv_layer=None, return_indices=True, *args, **kwargs):
        ...
        self.pins = pins
        phidden['conv_dim'] = phidden.get('conv_dim', len(checklist(pins['dim'])))
        ...
        phidden['batch_norm_conv'] = checklist(phidden.get('batch_norm_conv', True), self.depth)
    ...
    def init_modules(pins, phidden, return_indices=False, conv_layer=ConvLayer):
        ...
        for l in range(len(channels)-1):
            current_dict = {'kernel_size':phidden['kernel_size'][l], 'stride':phidden['stride'][l], 'output_padding':phidden['output_padding'][l], 'pool':phidden['pool'][l],
                            'padding':phidden['padding'][l], 'conv_dim':phidden['conv_dim'], 'batch_norm':phidden['batch_norm_conv'][l], 'dropout':phidden['dropout_conv'][l], 'dilation':phidden['dilation'][l]}
...
class ConvolutionalLatent(nn.Module):
    ...
    def __init__(self, pins, phidden, *args, **kwargs):
        ...
        * *conv_layer* (nn.Module) : Convolutional layer class
        ...
        # make post encoders
        self.post_encoder = self.get_post_mlp(mlp_input_size, phidden,  *args, **kwargs)
    ...
    def get_pooling_indices(self, layer=None):
        ...
        if layer is None:
            ...
            return self.conv_module.get_pooling_indices()[layer]
    ...
    def forward(self, x, *args, y=None, **kwargs):
        ...
        for i, inp in enumerate(x):
            ...
            if len(inp.shape) > self.conv_modules[i].conv_dim + 2:
                original_shape = inp.shape[:2]
                if not inp.is_contiguous():
                    inp = inp.contiguous()
                inp = inp.view(inp.shape[0]*inp.shape[1], *inp.shape[2:])

            # forward!
            conv_outs.append(self.transfer_modules[i](self.conv_modules[i](inp, y=y, *args, **kwargs)))
        conv_out = torch.cat(conv_outs, dim=1)
        out = self.post_encoder(conv_out, y=y)
        if original_shape:
            out = out.view(*original_shape, *out.shape[1:])
        return out
...
class DeconvolutionalLatent(ConvolutionalLatent):
    ...
    * *conv_layer* (nn.Module) : Convolutional layer class
    ...
    def __init__(self, pins, phidden, encoder=None, pouts=None,  *args, **kwargs):
        ...
        self.pins = pins; self.phidden = checklist(phidden, n=len(checklist(self.pins)), copy=True)
        ...
        for i, ph in enumerate(self.phidden):
            ...
            if has_pooling:
                ...
                pre_pooling_sizes_enc, post_pooling_sizes_enc = encoder.conv_modules[i].get_output_conv_length()
        ...
        if encoder is not None and self.has_pooling:
            self.encoder = encoder
        self.pre_mlp = self.get_pre_mlp(self.pins, {'dim':sum(transfered_sizes)}, *args, **kwargs)
    ...
    def forward(self, x, *args, y=None, **kwargs):
        ...
        for i in range(len(pre_output)):
            ...
            if self.has_pooling:
                pooling_indices = self.encoder.get_pooling_indices()[i]
