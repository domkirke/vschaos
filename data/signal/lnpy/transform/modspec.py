...
class AMS(BaseTransform):
    ...
    def __init__(self, first_transform=None,
                 second_transform=None, dynamic_range=60.):
...
#        return self.first_transform.to_string() + '_FFT2'
    ...
    def process(self, signal):
        ...
        print(signal)
        spec = librosa.stft(signal, n_fft=4096, hop_length=512)
        from matplotlib import pyplot as plt
        #plt.figure()
        #plt.imshow(librosa.logamplitude(np.real(spec)))
        #plt.axis('tight')
            #plt.imshow(X)
            #plt.axis('tight')
        # Logarithmic compression
        ...
        S = librosa.logamplitude(np.real(spec))
...
#            t_offset = spec_time[0]
        ...
        for i in range(0, S.shape[0] - nwin):
            print(i)
            ind = np.arange(i, i+nwin, dtype=np.int)
            X = (S[ind, :].T * win).T
