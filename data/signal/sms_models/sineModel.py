...
def sineModelAnal(x, fs, w, N, H, t, maxnSines = 100, minSineDur=.01, freqDevOffset=20, freqDevSlope=0.01):
	...
	while pin<pend:                                         # while input sound pointer is within sound            
		...
		tfreq = np.resize(tfreq, int(min(maxnSines, tfreq.size)))  # limit number of tracks to maxnSines
		tmag = np.resize(tmag, int(min(maxnSines, tmag.size)))     # limit number of tracks to maxnSines
		tphase = np.resize(tphase, int(min(maxnSines, tphase.size))) # limit number of tracks to maxnSines
		jtfreq = np.zeros(int(maxnSines))                          # temporary output array
		jtmag = np.zeros(int(maxnSines))                           # temporary output array
		jtphase = np.zeros(int(maxnSines))                         # temporary output array   
...
def sineModelSynth(tfreq, tmag, tphase, N, H, fs):
	...
	y = np.zeros(int(ysize))                                     # initialize output array
