...
def window_data(chunk, window=None, window_overlap=None, axis=0):
    dim = axis
    overlap = int(max(window_overlap * window, 1))
    n_windows = (chunk.shape[dim] - window) // overlap
    chunk_list = []
    if n_windows >= 0:
        for i in range(n_windows+1):
            chunk_list.append(np.expand_dims(np.take(chunk, range(i*overlap, i*overlap + window), axis=dim),dim))
    else:
        pads = [(0,0)]*len(chunk.shape)
        pads[dim] = (0, window - chunk.shape[dim])
        chunk_list = [np.pad(chunk, pads, mode="edge")[np.newaxis]]
    if len(chunk_list) > 2:
        return np.concatenate(chunk_list, axis=dim)
    else:
        return chunk_list[0]

def crop_or_pad(data, target_size):
    pad_dims = [(0,0)]*len(target_size)
    for i, dim in enumerate(target_size):
        if dim < data.shape[i+1]:
            data = np.take(data, range(dim), axis=i+1)
        elif dim > data.shape[i+1]:
            pad_dims[i] = (0, dim - data.shape[i+1])
    data = np.pad(data, [(0,0)]+pad_dims, mode="constant", constant_values=0)
    return data
